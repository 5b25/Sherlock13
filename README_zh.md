...
> **说明：** 当前 `main` 分支的代码为 **v1.0 稳定版**。我目前正在本地进行 v2.0 版本的架构重构。

**v2.0 版本特性：**
* **通信协议重构：** 从基于文本的字符串协议迁移至 **二进制 TLV (Type-Length-Value)** 协议，彻底解决 TCP 粘包与半包问题。
* **并发模型升级：** 从简单的“每客户端一线程”升级为 **线程池 (Thread Pool) + 任务队列** 模型，以提升高并发下的资源管理效率。
* **线程安全强化：** 引入更严格的互斥锁 (Mutex) 机制，消除服务端全局状态管理的竞态条件。
